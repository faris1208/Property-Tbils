...
export function AdminBuyers() {
  ...
  const fetch = async () => {
    setLoading(true);
    try {
      const r = await api.get('/admin/users/buyers');
      setRows(r.data.data || []);
  ...
  return (
    <div className="p-6 space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">All registered buyers on the platform.</p>
      </div>

      <div className="bg-white rounded-xl border overflow-x-auto">
        {loading ? <TableSkeleton cols={['Name', 'Email', 'Phone', 'Verified', 'Status', 'Joined', 'Actions']} /> : (
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b">
              <tr>{['Name', 'Email', 'Phone', 'Verified', 'Status', 'Joined', 'Actions'].map((h) => (
                <th key={h} className="text-left px-5 py-3 font-medium text-muted-foreground whitespace-nowrap">{h}</th>
              ))}</tr>
            </thead>
            <tbody className="divide-y">
              {rows.length === 0
                ? <tr><td colSpan={7} className="text-center py-16 text-muted-foreground">No buyers found.</td></tr>
                : rows.map((u) => (
